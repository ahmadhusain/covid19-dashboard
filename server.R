...
shinyServer(function(input, output) {
    ...
    ts_indo_longer %>%
      ...
      hchart(.,
             ...
             hcaes(x = datetime,
                   y = count,
                   group = variable)) %>%
      hc_title(text = "Statistics COVID-19 Outbreak in Indonesia") %>%
      hc_subtitle(text = "Source: https://kawalcorona.com/api/") %>%
  ...
  output$mapindo <- renderHighchart({
    hcmap(
      ...
      hc_colorAxis(minColor = "white",
                   ...
                   type = "logarithmic") %>%
      hc_exporting(enabled = TRUE) %>%
      hc_title(text = "Mapping Cumulative Confirmed Cases in Indonesia") %>%
      hc_subtitle(text = paste("Updated:", Sys.time())) %>%
      hc_mapNavigation(enabled = TRUE)
  ...
  googlenews <- reactive({
    ...
    for (i in 1) {
      ...
      detik_dummy <-
        data.frame("link" = link, "title" = title) %>%
  ...
  output$newstable <- renderDataTable({
    shiny::validate(need(input$getnews != "",
  ...
  observeEvent(input$select_button, {
    ...
    article_sentences <-
      tibble(text = googlenews()[selected_news, "text"]) %>%
